...
export default function Register() {
  ...
  const validateForm = () => {
    ...
    if (!form.phone.trim()) {
      ...
      newErrors.phone = "Doit commencer par 05, 06 ou 07 et contenir 10 chiffres";
      isValid = false;
    }
    ...
    if (!form.adresse.trim()) {
      ...
      newErrors.adresse = "L'adresse ne doit contenir que des lettres et des chiffres";
      isValid = false;
    }
  ...
  const SelectBox = ({ label, value, placeholder, onPress, error }) => (
    <View>
      ...
      <TouchableOpacity style={[styles.selectBox, error && styles.inputError]} onPress={onPress}>
  ...
  // ✅ INPUT WITH ERROR
  const InputWithError = ({ label, value, onChangeText, error, secureTextEntry, keyboardType }) => (
    <View>
      <Text style={styles.label}>{label}</Text>
      <TextInput

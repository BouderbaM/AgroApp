...
export default function Register() {
  ...
  // ✅ REGEX PATTERNS (قواعد التحقق)
  // يسمح بالحروف الفرنسية (بما فيها الحروف ذات العلامات مثل é, à) والمسافات وعلامة الشرطة
  const frenchTextRegex = /^[a-zA-ZÀ-ÿ\- ]+$/; 
  // يبدأ بـ 05 أو 06 أو 07 ويتبعه 8 أرقام على الأقل (المجموع 10 أرقام فأكثر)
  const phoneRegex = /^(05|06|07)\d{8,}$/; 
  // 8 أحرف/أرقام على الأقل ويجب أن يحتوي على حرف ورقم معاً
  const passwordRegex = /^(?=.*[A-Za-zÀ-ÿ])(?=.*\d)[A-Za-zÀ-ÿ\d]{8,}$/;
  ...
  const handleRegister = async () => {
    const { nom, prenom, email, phone, password, wilaya, role, commune, adresse } = form;

    // 1. التأكد من عدم ترك أي حقل فارغ
    if (!nom || !prenom || !email || !phone || !password || !wilaya || !role || !commune || !adresse) {
      Alert.alert("Erreur", "Veuillez remplir tous les champs obligatoires (*).");
      return;
    }

    // 2. التحقق من الاسم الأخير
    if (!frenchTextRegex.test(nom)) {
      Alert.alert("Erreur", "Le nom doit contenir uniquement des lettres françaises.");
      return;
    }

    // 3. التحقق من الاسم الأول
    if (!frenchTextRegex.test(prenom)) {
      Alert.alert("Erreur", "Le prénom doit contenir uniquement des lettres françaises.");
      return;
    }

    // 4. التحقق من البلدية
    if (!frenchTextRegex.test(commune)) {
      Alert.alert("Erreur", "La commune doit contenir uniquement des lettres françaises.");
      return;
    }

    // 5. التحقق من العنوان
    if (!frenchTextRegex.test(adresse)) {
      Alert.alert("Erreur", "L'adresse doit contenir uniquement des lettres françaises.");
      return;
    }

    // 6. التحقق من رقم الهاتف
    if (!phoneRegex.test(phone)) {
      Alert.alert("Erreur", "Le téléphone doit contenir au moins 10 chiffres et commencer par 05, 06 ou 07.");
      return;
    }
    ...
    if (!passwordRegex.test(password)) {
      Alert.alert("Erreur", "Le mot de passe doit contenir au moins 8 caractères (un mélange de lettres et de chiffres).");
      return;
    }

    // 9. التحقق من قبول السياسة
    if (!acceptPolicy) {
  ...
  // ✅ SELECT BOX
  const SelectBox = ({ label, value, placeholder, onPress, required }) => (
    <View>
  ...
  // مكون لإضافة علامة * للحقول العادية
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {/* PASSWORD */}
          <RequiredLabel text="Mot de passe" />
          <TextInput
            style={styles.input}
            secureTextEntry
            value={form.password}

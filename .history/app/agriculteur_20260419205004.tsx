...
export default function Agriculteur() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const totalConfirmed = docs.filter(d => d.status === "confirmed").length;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.listCard}>
          {stats.declared > 0 ? (
            <Text style={styles.listCardTitle}>Vous avez {stats.declared} déclarations</Text>
          ) : (
            <>
        ...
        <TouchableOpacity style={styles.listCard} onPress={() => router.push("/collect_points")}>
          <Text style={styles.listCardTitle}>Points de collecte disponibles</Text>
          <Text style={styles.listCardSub}>Consultez les points près de vous</Text>
        </TouchableOpacity>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  bottomNav: { position: "absolute", bottom: 0, left: 0, right: 0, height: 75, backgroundColor: "#1A1C1E", flexDirection: "row", justifyContent: "space-around", alignItems: "center", borderTopWidth: 1, borderTopColor: "#26292B", paddingBottom: Platform.OS === "ios" ? 15 : 0 },

...
export default function Register() {
  ...
  const validateForm = () => {
    // Regular Expressions
    const frenchTextRegex = /^[a-zA-ZÀ-ÿ\s'-]+$/; // يسمح بالحروف الفرنسية، المسافات، علامة التنصيص والشرطة
    const phoneRegex = /^(05|06|07)\d{8}$/; // يبدأ بـ 05 أو 06 أو 07 ويتكون من 10 أرقام
    const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/; // 8 أحرف/أرقام على الأقل ويحتوي على حرف ورقم
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // تحقق بسيط من الإيميل

    if (!form.prenom.trim() || !frenchTextRegex.test(form.prenom.trim())) {
      return "Le prénom est requis et ne doit contenir que des lettres françaises.";
    }
    if (!form.nom.trim() || !frenchTextRegex.test(form.nom.trim())) {
      return "Le nom est requis et ne doit contenir que des lettres françaises.";
    }
    if (!form.phone.trim() || !phoneRegex.test(form.phone.trim())) {
      return "Le téléphone doit contenir exactement 10 chiffres et commencer par 05, 06 ou 07.";
    }
    ...
    if (!form.commune.trim() || !frenchTextRegex.test(form.commune.trim())) {
      return "La commune est requise et ne doit contenir que des lettres françaises.";
    }
    if (!form.adresse.trim() || !frenchTextRegex.test(form.adresse.trim())) {
      return "L'adresse est requise et ne doit contenir que des lettres françaises.";
    }
  ...
  const handleRegister = async () => {
    // التحقق من قبول السياسة أولاً
    if (!acceptPolicy) {
      Alert.alert("Attention", "Veuillez accepter la politique de confidentialité");
      return;
    }
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        wilaya: form.wilaya,
        role: form.role,
        commune: form.commune, // ✅ تمت الإضافة
        adresse: form.adresse, // ✅ تمت الإضافة
  ...
  // ✅ SELECT BOX (تم تعديل ليقبل علامة *)
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {/* PRENOM */}
          <Text style={styles.label}>Prénom <Text style={styles.asterisk}>*</Text></Text>
          <TextInput
            style={styles.input}
            value={form.prenom}
          ...
          {/* NOM */}
          <Text style={styles.label}>Nom <Text style={styles.asterisk}>*</Text></Text>
          <TextInput
            style={styles.input}
            value={form.nom}
          ...
          {/* PHONE */}
          <Text style={styles.label}>Téléphone <Text style={styles.asterisk}>*</Text></Text>
          <TextInput
            style={styles.input}
            value={form.phone}
          ...
          {/* COMMUNE */}
          <Text style={styles.label}>Commune <Text style={styles.asterisk}>*</Text></Text>
          <TextInput
            style={styles.input}
            value={form.commune}
          ...
          {/* Adresse */}
          <Text style={styles.label}>Adresse <Text style={styles.asterisk}>*</Text></Text>
          <TextInput
            style={styles.input}
            value={form.adresse}
...
const styles = StyleSheet.create({
  ...
  brandSubTitle: { color: "#fff", fontSize: 18 },

  label: { color: "#fff", marginTop: 15, marginBottom: 6, fontSize: 14 },
  asterisk: { color: "red", fontWeight: "bold" }, // ✅ تنسيق علامة النجمة
  ...
  policyLink: { color: "#4CAF50", fontSize: 12, marginTop: 4 },

...
export default function DeclarationsList() {
  ...
  const handleCollect = async (item: any) => {
    try {
      ...
      await addDoc(collection(db, "collections"), {
        ...
        etat: item.etat,
        point_collecte: item.point_collecte,
        prixTotal: item.prixTotal ?? 0,
        date_collecte: item.date_collecte,
        createdAt: serverTimestamp(),
        status: "collecté",
      });

      // 2️⃣ Delete from declarations
      await deleteDoc(doc(db, "declarations", item.id));

      // 3️⃣ Update UI instantly
      setData((prev) => prev.filter((d) => d.id !== item.id));
      setFiltered((prev) => prev.filter((d) => d.id !== item.id));

      Alert.alert("Succès", "Déclaration collectée ✔");
    } catch (error: any) {
  ...
  const renderItem = ({ item }: any) => (
    <View style={styles.card}>
      ...
      >
        <Text style={{ color: "#fff", fontWeight: "bold" }}>🚛 Collecter</Text>
      </TouchableOpacity>
